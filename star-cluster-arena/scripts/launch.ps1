...
$runtimeRoot = Join-Path $env:LOCALAPPDATA "StarClusterArena"
...
function Test-BallArenaServer {
  try {
    ...
    return $health.ok -eq $true -and $health.name -eq "star-cluster-arena"
  } catch {
    return $false
  }
}
...
try {
  ...
  Write-Host "Star Cluster Arena launch failed:" -ForegroundColor Red
